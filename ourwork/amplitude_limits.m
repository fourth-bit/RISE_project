%Amplitude limits (low-end) in ACD
...
for i=1:11
    ...
    %d1.plot_osc_trg;
    % Plots the waveform and the synchronicity
    %d1.plot_osc_rho;
    %title(sprintf('qmax: %f', qmax));

    avg_rhos(i) = sum(d1.model.rho(d1.nstart:end)) / (d1.model.nsamples - d1.nstart + 1);
    ...
    %figure
    % Plot synchronicity of electrodes
    %d1.plot_rho_energy
